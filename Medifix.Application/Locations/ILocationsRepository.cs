...
{
    Task<Result<List<Location>>> GetByIdWithParentsAsync(LocationId locationId,
        CancellationToken cancellationToken = default);

    Task<Result<List<Location>>> GetChildren(LocationId locationId, CancellationToken cancellationToken = default);
    Task<Result<bool>> SameTypeAndNameAlreadyExist(Location location, CancellationToken cancellationToken = default);
}